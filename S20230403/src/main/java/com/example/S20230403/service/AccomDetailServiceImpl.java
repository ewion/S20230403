...
import com.example.S20230403.Dao.AccomDetailDao;
...
public class AccomDetailServiceImpl implements AccomDetailService {
	...
	public List<JinJoin> getAccomDetailList(String biz_id, String checkIn, String checkOut) {
		...
		//LocalDate로 파싱
		LocalDate LocalCheckIn = LocalDate.parse(checkIn, formatter);
		LocalDate LocalCheckOut = LocalDate.parse(checkOut, formatter);
		...
		for(JinJoin roomPrice : accomDetail) {
	...
	public List<Review> getAccomReviewWithImagesList(String biz_id) {
		...
		for (Review review : accomReview) {
	        ...
	        //Review객체에 생성한 reviewImages(pay_id가 일치하는 이미지만 저장하는 필드)에 setter로 현재 p
	...
	public Review getCalculateReview(String biz_id) {
		...
		//Math는 Long으로 받아서 100.0으로 다시 나눠야함 
		double calAvgReview = Math.floor((calculateReview.getAvgReview() * 100) / 100.0 ); 
